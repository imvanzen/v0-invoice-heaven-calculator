...
- Accumulated limit validation (ML annual, MC bi-monthly, Integrations quarterly)
- Export calculations to JSON file for backup
- Import calculations from JSON file with merge/replace/skip strategies
- Period-based input restrictions (MC only in settlement months, Integrations only in quarter months)
...
- Integrations limit fixed at 1,500 PLN quarterly (not employment date dependent)
...
- Input fields for all benefit categories (ML, MC, Budget, Integrations, Other)
...
- Narzędzia: Equipment and software purchases
